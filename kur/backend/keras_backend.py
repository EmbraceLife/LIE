...
class KerasBackend(Backend):
    ...
    def compile(self, model, loss=None, optimizer=None, blocking=True,
                assemble_only=False):
        """ 1. store the compiled model inside model.compile['raw']; 2. further process loss, update using keras; 3. additional datasets are handled here; Returns the Keras model instance.
        """

        # model.compiled is no more None
        if model.compiled is None:
            model.compiled = {}

        # make a model using keras with input and output
        # compiled model looks very different from just parsed model
        if 'raw' not in model.compiled:
            ...
            # store compiled into model.compiled['raw']
        ...
        # assemble_only is an argument for keras.backend module
        import keras.backend as K				# pylint: disable=import-error

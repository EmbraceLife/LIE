...
## /Users/Natsume/Downloads/LIE/VGG16/vgg16.py(49)
...
#   """
#   # only two options for weights: either imagenet or None for start from nothing
#   if weights not in {'imagenet', None}:
#     raise ValueError('The `weights` argument should be either '
#                      '`None` (random initialization) or `imagenet` '
#                      '(pre-training on ImageNet).')
#
#   # if use imagenet weights, then must use 1000 classes
#   if weights == 'imagenet' and include_top and classes != 1000:
#     raise ValueError('If using `weights` as imagenet with `include_top`'
#                      ' as true, `classes` should be 1000')
#
#   # Determine proper input shape (224, 224, 3): based default_size 224, channel_last
...
#
#   # if input_tensor is not given upfront, build an Input layer with just shape
#   if input_tensor is None:
#     img_input = Input(shape=input_shape)
#   else:
# 	# if input_tensor is given upfront, build an Input layer with shape and tensor

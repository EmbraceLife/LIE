...
# pprint(data.train.__dict__) # see attr
# pprint(getmembers(data.train)) # see all attr and methods

# create an additional attr
...
# from x 2-dimto x images 4-dim box
x_image = tf.reshape(x, [-1, img_size, img_size, num_channels])

# true labels box: 2-dim
y_true = tf.placeholder(tf.float32, shape=[None, 10], name='y_true')

# true class box: 1-dim
y_true_cls = tf.argmax(y_true, dimension=1)

# layer_conv1 box and its weight box
...
# layer_conv2 box and its weight box
...
# flatten layer box and its num_features box
...
def optimize(num_iterations):
    ...
    for i in range(total_iterations,  # maybe we could try progress bar here
		...
		# next_batch() only works within session()
